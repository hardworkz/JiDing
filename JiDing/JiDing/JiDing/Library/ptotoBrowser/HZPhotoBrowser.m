...
- (void)hidePhotoBrowser:(UITapGestureRecognizer *)recognizer
{
    ...
    UIView *sourceView;
    if (self.isCenterHide) {
        sourceView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, 0, 0)];
    }else
    {
    ...
    [UIView animateWithDuration:HZPhotoBrowserHideImageAnimationDuration animations:^{
        if (self.isCenterHide) {
            tempImageView.frame = CGRectMake(SCREEN_Width * 0.5, SCREEN_Height * 0.5, 0, 0);
        }else{
