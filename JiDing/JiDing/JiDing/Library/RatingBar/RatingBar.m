...
-(void)tap:(UITapGestureRecognizer *)gesture{
    if(self.enable){
        ...
        if ([self.delegate respondsToSelector:@selector(ratingBar:didTapToNumber:)]) {
            [self.delegate ratingBar:self didTapToNumber:_starNumber];
        }
    }
}
-(void)pan:(UIPanGestureRecognizer *)gesture{
    if(self.enable){
        ...
        if ([self.delegate respondsToSelector:@selector(ratingBar:didPanToNumber:)]) {
            [self.delegate ratingBar:self didPanToNumber:_starNumber];
        }
    }
}
