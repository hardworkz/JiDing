...
- (void)viewDidLoad {
    ...
    [self setNavTitle];
    [self.navigationController.navigationBar setBarTintColor:AppMainColor];
    // 透明度设置为0.3
...
- (void)setNavTitle
{
    ...
    self.title = [NSString stringWithFormat:@"%ld/9",self.selectedAssetArray.count];
}
...
- (void)setUpProperties
{
    ...
    preBtn.frame = CGRectMake(0, 0, 80, 30);
    [preBtn setTitle:@"预览所选" forState:UIControlStateNormal];

    [preBtn setTitleColor:[JKUtil getColor:@"ffffff"] forState:UIControlStateNormal];
    [preBtn setTitleColor:[JKUtil getColor:@"eeeeee"] forState:UIControlStateHighlighted];
    [preBtn setTitleColor:[JKUtil getColor:@"ffffff"] forState:UIControlStateDisabled];
...
- (void)showAssetsGroupView
{
    ...
    [UIView animateWithDuration:0.3f
                     animations:^{
                         self.assetsGroupsView.top = self.view.height - self.assetsGroupsView.height - 44;
...
- (void)hideAssetsGroupView
{
    [UIView animateWithDuration:0.3f
                     animations:^{
                         self.assetsGroupsView.top = self.view.height;
                         self.overlayView.alpha = 0.0f;
                     }completion:^(BOOL finished) {
...
//    self.finishLabel.text = [NSString stringWithFormat:@"下一步(%ld)",self.selectedAssetArray.count];
    self.finishLabel.text = [NSString stringWithFormat:@"完成"];
...
- (JKAssetsGroupsView *)assetsGroupsView{
    if (!_assetsGroupsView) {
        _assetsGroupsView = [[JKAssetsGroupsView alloc] initWithFrame:CGRectMake(self.view.width * 0.25, self.view.height, self.view.width * 0.5, 60 * 3)];
        _assetsGroupsView.layer.masksToBounds = YES;
        _assetsGroupsView.layer.cornerRadius = 10;
...
- (UIToolbar *)toolbar{
    if (!_toolbar) {
        _toolbar = [[UIToolbar alloc] initWithFrame:CGRectMake(0, self.view.height-44-64, self.view.width, 100)];
//        _toolbar.backgroundColor = [UIColor whiteColor];
        _toolbar.tintColor = [JKUtil getColor:@"ffffff"];
        if ([_toolbar respondsToSelector:@selector(barTintColor)]) {
            _toolbar.barTintColor = [JKUtil getColor:@"ffffff"];
        }
...
//        UIImage  *imgSelect = [UIImage imageNamed:@"compose_photo_original_highlighted"];
        ...
        _selectButton.frame = CGRectMake(0, floor((44-img.size.height)/2), img.size.width, img.size.height);
...
//        [_selectButton addTarget:self action:@selector(selectOriginImage) forControlEvents:UIControlEventTouchUpInside];
        [_selectButton addTarget:self action:@selector(cancelEventDidTouched) forControlEvents:UIControlEventTouchUpInside];
        
        [_toolbar addSubview:_selectButton];
        
        UILabel  *label = [[UILabel alloc] init];
        label.backgroundColor = [UIColor clearColor];
        label.textColor = [JKUtil getColor:@"fa9b98"];
        ...
        _finishLabel.textColor = [JKUtil getColor:@"f34842"];
...
//        [_finishButton setBackgroundImage:[JKUtil stretchImage:[UIImage imageNamed:@"picker_button_orange_highlighted"] capInsets:UIEdgeInsetsMake(5, 5, 5, 5) resizingMode:UIImageResizingModeStretch] forState:UIControlStateHighlighted];
        ...
        _titleButton = [UIButton buttonWithType:UIButtonTypeCustom];
        _titleButton.frame = CGRectMake(self.view.width / 3, _selectButton.frame.origin.y, 120, 30);

        UIImage  *img1 =[UIImage imageNamed:@"navigationbar_title_highlighted"];
        [_titleButton setBackgroundImage:nil forState:UIControlStateNormal];
        [_titleButton setBackgroundImage:[JKUtil stretchImage:img1 capInsets:UIEdgeInsetsMake(5, 2, 5, 2) resizingMode:UIImageResizingModeStretch] forState:UIControlStateHighlighted];
        [_titleButton addTarget:self action:@selector(assetsGroupDidSelected) forControlEvents:UIControlEventTouchUpInside];
        
        UIImage  *img2 = [UIImage imageNamed:@"navigationbar_arrow_down"];
        UIImage  *imgSelect2 = [UIImage imageNamed:@"navigationbar_arrow_up"];
        _arrowImageView = [UIButton buttonWithType:UIButtonTypeCustom];
        _arrowImageView.frame = CGRectMake(2, 0, img2.size.width, img2.size.height);
        [_arrowImageView setBackgroundImage:img2 forState:UIControlStateNormal];
        [_arrowImageView setBackgroundImage:imgSelect2 forState:UIControlStateSelected];
        [self.titleButton addSubview:_arrowImageView];
        
        [_toolbar addSubview:_titleButton];
